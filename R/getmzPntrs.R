  ...
  for ( chromatogram_input_index_num in seq(1, length(filenames$runs)) ){
    tryCatch(
      expr = {
        ...
        message(sprintf("Cacheing mzML for %s (%s) of %s runs: Elapsed Time = %s sec", run, current_filename, length(filenames$runs), round(exec_time$toc - exec_time$tic, 3) )) 
